...
class ClientConfigGeneratorUnitTest : TestCase() {
    ...
    fun testBuildClaudeCodeCommandContainsRemoveCommand() {
        val command = ClientConfigGenerator.buildClaudeCodeCommand(
            serverUrl = "http://127.0.0.1:63342/mcp/sse",
    ...
    fun testBuildClaudeCodeCommandContainsAddCommand() {
        val command = ClientConfigGenerator.buildClaudeCodeCommand(
            serverUrl = "http://127.0.0.1:63342/mcp/streamable-http",
            serverName = "test-server"
        )

        assertTrue(
            "Command should contain add command",
            command.contains("claude mcp add --transport http test-server http://127.0.0.1:63342/mcp/streamable-http --scope user")
        )
    }
    ...
    fun testBuildClaudeCodeCommandFormat() {
        val command = ClientConfigGenerator.buildClaudeCodeCommand(
            serverUrl = "http://127.0.0.1:63342/mcp/streamable-http",
            serverName = "intellij-idesense"
        )

        val expectedCommand = "claude mcp remove intellij-idesense 2>/dev/null ; " +
            "claude mcp add --transport http intellij-idesense http://127.0.0.1:63342/mcp/streamable-http --scope user"
    ...
    fun testBuildCodexCommandContainsAddCommand() {
        ...
        assertTrue(
            ...
            command.contains("codex mcp add test-server --url http://127.0.0.1:63342/mcp/streamable-http")
        )
    }
