...
internal data class PopupClassSearchCandidate(
    val item: NavigationItem,
    val fullName: String?
)

internal data class PopupClassSearchResult(
    val candidates: List<PopupClassSearchCandidate>,
    val isQualifiedQuery: Boolean,
    val localPattern: String
)

/**
 * Headless wrapper around the same model/provider stack used by IntelliJ's Go to Class popup.
 *
 * Mirrors [PopupFaithfulSymbolSearch] but drives [GotoClassModel2] (Ctrl+N / Cmd+O) instead
 * of [com.intellij.ide.util.gotoByName.GotoSymbolModel2]. Preserves popup query parsing,
 * full-name matching, and result ordering while staying usable from MCP tool execution
 * without constructing any UI.
 */
internal object PopupFaithfulClassSearch {

    private val LOG = logger<PopupFaithfulClassSearch>()
    ...
    fun search(
        ...
        limit: Int
    ): PopupClassSearchResult {
        val disposable = Disposer.newDisposable("PopupFaithfulClassSearch")
        try {
            ...
            val candidates = mutableListOf<PopupClassSearchCandidate>()
            ...
            if (provider is ChooseByNameInScopeItemProvider) {
                provider.filterElementsWithWeights(viewModel, parameters, indicator) { found ->
                    ...
                    candidates.add(PopupClassSearchCandidate(item = item, fullName = model.getFullName(item)))
                ...
                provider.filterElements(viewModel, pattern, scope.isSearchInLibraries, indicator) { item ->
                    ...
                    candidates.add(PopupClassSearchCandidate(item = navigationItem, fullName = model.getFullName(navigationItem)))
                    candidates.size < limit
                }
            }

            return PopupClassSearchResult(
    ...
    private fun resolveSearchContext(project: Project): PsiElement? = runCatching {
        ...
        LOG.debug("Failed to resolve current editor context for popup-backed class search", error)
    }.getOrNull()
