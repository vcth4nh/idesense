...
response shapes changed. Read **Breaking** before upgrading.

### Breaking
- **`ide_find_references` is renamed to `ide_find_usages`** — aligns with the IntelliJ "Find Usages" action (`FindUsagesHandlerFactory`). Schema, parameters, and return shape are otherwise unchanged; update any hard-coded `ide_find_references` calls.
- **`matchMode` replaced by `fuzzySearch` on `ide_find_class` and `ide_find_symbol`.** The `matchMode` enum (`substring | prefix | exact | camelCase`) is removed; pass `fuzzySearch` (boolean, default `false`) instead. `false` = exact, case-insensitive name match; `true` = the IDE's fuzzy matching (camelCase + substring). **Both tools now default to exact** (previously fuzzy/substring). Calls still passing `matchMode` fail strict validation with `invalid_arguments` / `unknown_parameter`.
- **`language` + `symbol` parameters removed** from `ide_find_usages`, `ide_find_definition`, `ide_find_implementations`, `ide_find_super_methods`, and `ide_call_hierarchy`. Resolve by `file` + `line` + `column` instead.
- **Hierarchy `scope` vocabulary changed** to the IDE's native values, **on `ide_call_hierarchy` and `ide_type_hierarchy` only** — call hierarchy: `all`, `production`, `test`, `this_class`, `this_module`; type hierarchy: `all`, `production`, `test`. The previous `project_files` / `project_and_libraries` / `project_production_files` / `project_test_files` values are no longer accepted on these two tools (the other search tools keep them). (`production` returns empty in projects without production source roots — same as the IDE's "Production" tab.)
- **Unified per-item response shape across all 8 navigation tools** (`ide_find_usages`, `ide_find_definition`, `ide_find_class`, `ide_find_symbol`, `ide_find_implementations`, `ide_find_super_methods`, `ide_type_hierarchy`, `ide_call_hierarchy`). Contract: `name`, `qualifiedName?`, `enclosingScope?`, `kind`, `file`, `line`, `column`, plus tool-specific `preview` / `usageType` / `supertypes` / `children` (order: identity → location → recursion). Closes #15, #13, #10, #9.
  - **Renamed:** `type` → `usageType` (find_usages); `astPath` → `enclosingScope` (find_usages, find_definition); `symbolName` → `name` (find_definition).
  - **Removed:** `language` (every nav tool — derivable from the file extension); `containerName` (find_class, find_symbol); `signature`, `containingClass`, `containingClassKind`, `isInterface`, `depth` (find_super_methods).
  - **Added:** `kind` on call-hierarchy items; `line` / `column` on type-hierarchy items; `enclosingScope` fallback (present only when `qualifiedName` is null) on find_definition / type_hierarchy / call_hierarchy.
- **`qualifiedName` now matches IntelliJ's "Copy Reference"** (delegates to the `QualifiedNameProvider` EP) across every FQN-producing tool. Java method/field FQNs use `#` and include parameter signatures — e.g. `com.example.Foo.bar` → `com.example.Foo#bar(java.lang.String)` — so overloads are now distinct; Java *class* FQNs are unchanged. Rust/PHP edge cases the provider can't resolve now return `null` (was a simple name). `qualifiedName` has always been nullable — keep handling `null`. Closes #3.
- **`ide_type_hierarchy` / `ide_call_hierarchy` `column` now points at the name identifier** (previously the `class` / `def` / `func` keyword), matching `ide_find_class` / `ide_find_symbol` and Go-to-Declaration.
- **`ide_file_structure` output format changed.** The formatted string now renders from the IDE's own `ItemPresentation` (name + signature) with an explicit modifier prefix (e.g. `public abstract Shape (line 10)`); the previous per-node `kind` / `modifiers` fields and `class ` / `def ` prefixes are gone. Clients parsing the old shape must adjust.

### Added
- **`ide_install_plugin` and `ide_restart` tools** (universal, disabled by default) — install a locally built plugin `.zip` into the IDE's plugins directory and restart the IDE, enabling a fully headless plugin dev loop.
- **`ide_type_hierarchy` now accepts `direction` (`supertypes` | `subtypes` | `both`, default `both`) and `maxDepth` (default 5, max 20)**, reaching parity with `ide_call_hierarchy`. `both` returns the union of the supertypes and subtypes walks — deliberately not the IDE's single merged tree, which several language browsers cannot build headlessly (Kotlin's K2 browser asserts the EDT, GoLand returns null, RustRover ships no type-hierarchy provider).
- **`ide_find_super_methods` now supports Rust and Go** (registered in RustRover and GoLand for the first time) and **resolves more anchor kinds**, mirroring Go-to-Super (Ctrl+U): class / interface / struct / trait declarations → direct supertypes; Java lambdas → the single abstract method they implement; TypeScript field / constant → the overridden member. Closes #21, #22, #24, #26.
- **`ide_file_structure` is now universal** — works in any language with a registered structure-view factory (adds Go, PHP, Rust, and others), no longer gated on five hand-written handlers. New **`show`** parameter (select which categories/filters are visible, per language) and **`sort`** parameter (`"visibility"`).
...
- **Server-side argument validation** — every `tools/call` is validated against the target tool's input schema before execution. Missing required parameters, unknown/misspelled keys, wrong primitive types, and out-of-enum values are rejected with a structured `invalid_arguments` error that aggregates **all** violations in one response (each names the offending parameter; unknown-key errors list the accepted parameters). Numeric-string coercion stays lenient (`line: "123"` is accepted). Closes #12.
- **Live MCP test harness** in `live-test/` — snapshot-based regression suite across 8 languages (Python, Java, Kotlin, JavaScript, TypeScript, Go, PHP, Rust) and the navigation/intelligence tools.

### Changed
- **`ide_find_class` / `ide_find_symbol` default to exact (case-insensitive) name matching**; opt into the IDE's fuzzy matching with `fuzzySearch: true` (see Breaking).
- **MCP protocol version is negotiated at `initialize`** (supported: 2025-11-25, 2025-03-26, 2024-11-05) instead of a fixed per-transport version; streamable-HTTP now defaults to 2025-11-25.
- **All tool errors are structured** `{ "error": "<snake_case_code>", "message": "…", … }` objects (carried in both the text mirror and native `structuredContent`), replacing bare-text error strings. The dispatch catch-all distinguishes client-caused failures (stable codes such as `index_not_ready` / `file_not_found`, no longer logged at ERROR) from genuine internal failures (`internal_error`, still logged at ERROR). `ProjectResolver` errors (`no_project_open` / `project_not_found` / `multiple_projects_open`) also carry `structuredContent`. Closes #4.
- **`ide_find_usages` now routes through `FindUsagesHandlerFactory`** (the Alt+F7 equivalence class) instead of raw `ReferencesSearch`, recovering references the old path missed — Python stdlib skeleton↔typeshed, Java `.class`↔source, Kotlin light-classes. Caret on a constructor call or rebinding site no longer narrows the result set.
- **`ide_find_definition` resolves via `GotoDeclarationHandler` first** (Ctrl+Click semantics) before falling back to `PsiReference.resolve()`, so language navigation rules (e.g. constructor call → class) are honored.
- **`ide_find_implementations` now delegates to the `DefinitionsScopedSearch` EP** (Ctrl+Alt+B), working for all languages including Go, and also returns lambdas / method references to `@FunctionalInterface` types (`kind: "LAMBDA"` / `"METHOD_REFERENCE"`).
- **`ide_find_super_methods` now delegates to each language's own super-method API** and returns the **full transitive chain for JavaScript/TypeScript** (was immediate-parent only). Results may surface members the previous walkers missed — Kotlin properties / primary-constructor `val`/`var` / `expect`-`actual` / `internal`; PHP trait sources / `insteadof` / `@mixin` / constants; Python `@property` getter/setter direction. Closes #19, #23.
- **Semantic `kind` across tools** — PHP interfaces/traits/enums, Java abstract classes/records, Go structs/interfaces, and Kotlin data/sealed/object now report `INTERFACE` / `TRAIT` / `ENUM` / `ABSTRACT_CLASS` / `RECORD` / `STRUCT` / `DATA_CLASS` / `SEALED_CLASS` where they previously collapsed to `CLASS`.
- **`qualifiedName` now populates for Go and Rust elements** (neither plugin registers a `QualifiedNameProvider`; filled via dedicated fallbacks).
- **`ide_call_hierarchy` callee results now include constructor / `new` / struct-literal calls** (Java, Kotlin, JS/TS, Python, Rust) that were previously dropped by over-narrow PSI traversal.

### Fixed
- **`ide_call_hierarchy` / `ide_type_hierarchy`** no longer break the IDE's interactive Hierarchy tool window in remote-dev / `serverMode`, and no longer return a spurious `"Browser refused element …"` error when a write action goes pending mid-read. Closes #17, #141.
- **`ide_call_hierarchy` `scope` is now honored in Go** — GoLand's browser reads its own `myScope` field, which our headless scope poke previously didn't set, so `production` / `test` / `this_class` were ignored.
- **`ide_find_super_methods` (Go)** returns an empty hierarchy (not a "No method found" error) for a method that satisfies no interface, matching the Java/Kotlin/Python behavior. Closes #25.
- **Go interface-method `qualifiedName`** now includes the enclosing interface (`fmt.Stringer.String`) instead of being indistinguishable from a top-level function. Closes #20.
- **Numerous per-language correctness fixes** across `ide_type_hierarchy`, `ide_call_hierarchy`, `ide_find_implementations`, and `ide_file_structure` (Java/Kotlin/Python/JS-TS/PHP/Go/Rust): proper display names for anonymous classes and enum constants; JDK supertypes (`java.lang.Enum` / `Record`) included regardless of search scope; direct-vs-transitive subtype correctness (JS); nested functions/classes surfaced in structure view (Python); and assorted per-language `kind` / `qualifiedName` accuracy.
- **`serverInfo.version`** in the `initialize` response now reflects the actual installed plugin version (was hardcoded to `4.10.4`).

### Removed
- Internal architecture cleanup (no wire-format impact): the per-language `LanguageHandler` / `StructureHandler` / `SymbolSearchHandler` registries and their implementations were replaced by IntelliJ `LanguageExtension` extension points (`languageKindResolver`, `superMethodsProvider`, declared in `*-features.xml`), and `ide_file_structure` now delegates to the platform's `LanguageStructureViewBuilder` — net ~3,000 LOC of bespoke plugin code removed.
